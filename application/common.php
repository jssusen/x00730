...
function cleanDomin($data)
{
    foreach ($data as $k=>$v)
    {
        $data["headimage"]="/".stristr($data["headimage"],"uploads");
        $data["licenseimage"]="/".stristr($data["licenseimage"],"uploads");
//        unset($data["version"]);
        if ($k=="identityimages")
        {
            $data["identityimages"]=explode(',',$data["identityimages"]);
            foreach ($data["identityimages"] as $key=>$val)
            {
                $data["identityimages"][$key]="/".stristr($val,"uploads");
            }
            $data["identityimages"]=implode(",",$data["identityimages"]);
        }
...
function cleanUrl($data){
    $data["collectionimage"]="/".stristr($data["collectionimage"],"uploads");
    return $data;
}
...
function saltPassword($password){
    return md5("dianFeng@".$password);
}
//生成token
function createrToken(){
  return   sprintf(
...
function userInfo($param = [],$field="*")
{
    ...
    $user_info = \app\apis\model\Member::where('id', 1)->field($field)->find();
    if ($user_info){
...
//            \app\apis\model\Member::where('id', $user_info['id'])->update(['token' => '']);
        ...
        return ['status' => 0, 'msg' => 'token不存在', 'data' => []];
    }
}
