...
<script type="text/javascript">
    ...
    <div class="form-group">
        ...
        <div class="col-xs-12 col-sm-8">
            <input id="c-match_time" data-rule="required" class="form-control datetimepicker" data-date-format="YYYY-MM-DD HH:mm:ss" data-use-current="true" name="row[match_time]" type="text" value="<?php echo $row['match_time']?datetime($row['match_time']):''; ?>">
        </div>
    </div>
    <div class="form-group">
    ...
    <div class="form-group">
        ...
        <div class="col-xs-12 col-sm-8">
            <input id="c-unmatched" data-rule="required" class="form-control" step="0.01" name="row[unmatched]" type="number" value="<?php echo htmlentities($row['unmatched']); ?>">
        </div>
    </div>
    <div class="form-group">
        ...
        <div class="col-xs-12 col-sm-8">
            ...
            <select  id="c-order_type" data-rule="required" class="form-control selectpicker" name="row[order_type]">
                <?php if(is_array($orderTypeList) || $orderTypeList instanceof \think\Collection || $orderTypeList instanceof \think\Paginator): if( count($orderTypeList)==0 ) : echo "" ;else: foreach($orderTypeList as $key=>$vo): ?>
                    <option value="<?php echo $key; ?>" <?php if(in_array(($key), is_array($row['order_type'])?$row['order_type']:explode(',',$row['order_type']))): ?>selected<?php endif; ?>><?php echo $vo; ?></option>
                <?php endforeach; endif; else: echo "" ;endif; ?>
            </select>
    ...
    <div class="form-group">
        ...
        <div class="col-xs-12 col-sm-8">
            ...
            <label for="row[match_status]-<?php echo $key; ?>"><input id="row[match_status]-<?php echo $key; ?>" name="row[match_status]" type="radio" value="<?php echo $key; ?>" <?php if(in_array(($key), is_array($row['match_status'])?$row['match_status']:explode(',',$row['match_status']))): ?>checked<?php endif; ?> /> <?php echo $vo; ?></label> 
            <?php endforeach; endif; else: echo "" ;endif; ?>
            </div>
    ...
    <div class="form-group">
        ...
        <div class="col-xs-12 col-sm-8">
            <input id="c-pay_status" data-rule="required" class="form-control" name="row[pay_status]" type="number" value="<?php echo htmlentities($row['pay_status']); ?>">
        </div>
    </div>
    <div class="form-group">
        ...
        <div class="col-xs-12 col-sm-8">
            ...
            <select  id="c-is_lock" data-rule="required" class="form-control selectpicker" name="row[is_lock]">
                <?php if(is_array($isLockList) || $isLockList instanceof \think\Collection || $isLockList instanceof \think\Paginator): if( count($isLockList)==0 ) : echo "" ;else: foreach($isLockList as $key=>$vo): ?>
                    <option value="<?php echo $key; ?>" <?php if(in_array(($key), is_array($row['is_lock'])?$row['is_lock']:explode(',',$row['is_lock']))): ?>selected<?php endif; ?>><?php echo $vo; ?></option>
                <?php endforeach; endif; else: echo "" ;endif; ?>
            </select>
    ...
    <div class="form-group">
        ...
        <div class="col-xs-12 col-sm-8">
            <input id="c-source" data-rule="required" class="form-control" name="row[source]" type="text" value="<?php echo htmlentities($row['source']); ?>">
        </div>
    </div>
    <div class="form-group">
        <label class="control-label col-xs-12 col-sm-2"><?php echo __('Plan_time'); ?>:</label>
        <div class="col-xs-12 col-sm-8">
            <input id="c-plan_time" class="form-control datetimepicker" data-date-format="YYYY-MM-DD HH:mm:ss" data-use-current="true" name="row[plan_time]" type="text" value="<?php echo $row['plan_time']?datetime($row['plan_time']):''; ?>">
        </div>
    </div>
    <div class="form-group">
